/**
 * Pre-fetch Open-Meteo Historical Weather (ERA5) daily means for US wine regions.
 * Reads all src/data/*region*.json files. Output: public/data/climate.json (°C)
 */
...
async function main() {
  ...
  for (const r of results) {
    console.log(
      `  ${r.id.padEnd(28)} ann=${r.annual_mean_c}°C  grow=${r.growing_season_mean_c}°C  (${r.station})`,
    )
  }
}
